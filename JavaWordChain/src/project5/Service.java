...
public class Service extends Thread {
				...
				if (msg.trim().length() > 0) {
					...
					case "start":
						...
						for (int i = 0; i < myRoom.userV.size(); i++) {// 벡터 인덱스
							Service user = myRoom.userV.get(i); // 각각의 클라이언트 얻어오기
							if (user.state == 0) {
								messageTo("300|----모두 준비해야 시작할 수 있습니다.----");
								flag = 1;
								break;
							}
						}
						if (flag == 0){
							messageAll("300|---- 게임이 시작되었습니다. ----");
	...
	public String getRoomScore() {// 같은방의 인원정보
		String str = "";
		for (int i = 0; i < myRoom.userV.size(); i++) {
			// "길동,라임,주원"
			Service ser = myRoom.userV.get(i);
